...
// districts. Single source of truth for both apps/web (city/district
// picker, "nearby workplaces" sort) and apps/api (validating that an
// admin-entered Company.city/district actually names a real place, rather
// than accepting arbitrary free text that then can't be found by any filter
// — see CompaniesService.createByAdmin). Previously this table only existed
// client-side, which meant nothing ever checked a submitted city/district
// against it before writing to the database. Source: province/district
// names and centroid lat/lng adapted from
// https://github.com/enisbt/turkey-cities (public domain city list),
// re-cased to standard Turkish capitalization.
...
// Loose match so admin-typed Company.city/district values (any case/
// diacritics inconsistency) still resolve to a canonical province — used
// both for display-side matching (web) and write-time validation (api),
// so the two can never drift into checking different rules.
...
// Resolves a district name against a specific province's real district list
// (case/diacritic-insensitive, same normalizeCityName fold) — used at
// company-creation time to reject a district that doesn't actually belong to
// the given province, e.g. "Kadıköy" under "Ankara". Returns the district's
// canonical spelling (not whatever casing was submitted) so what's stored
// always matches what the picker itself would show.
export function findDistrictInProvince(province: TurkeyProvince, districtName: string): string | null {
  const normalized = normalizeCityName(districtName);
  return province.districts.find((d) => normalizeCityName(d) === normalized) ?? null;
}
