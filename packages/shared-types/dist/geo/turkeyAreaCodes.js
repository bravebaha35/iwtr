...
exports.ALL_TURKEY_AREA_CODES = exports.TURKEY_AREA_CODES_BY_PLATE = void 0;
exports.provinceForAreaCode = provinceForAreaCode;
exports.areaCodesForProvince = areaCodesForProvince;
...
exports.TURKEY_AREA_CODES_BY_PLATE = {
...
exports.ALL_TURKEY_AREA_CODES = Array.from(new Set(Object.values(exports.TURKEY_AREA_CODES_BY_PLATE).flat())).sort();
const PLATE_BY_NORMALIZED_PROVINCE_NAME = new Map(turkey_1.TURKEY_PROVINCES.map((p) => [(0, turkey_1.normalizeCityName)(p.name), p.plate]));
const PLATE_BY_AREA_CODE = new Map(Object.entries(exports.TURKEY_AREA_CODES_BY_PLATE).flatMap(([plate, codes]) => codes.map((code) => [code, plate])));
const PROVINCE_BY_PLATE = new Map(turkey_1.TURKEY_PROVINCES.map((p) => [p.plate, p]));
...
function provinceForAreaCode(areaCode) {
...
function areaCodesForProvince(provinceName) {
    if (!provinceName)
        return null;
    const plate = PLATE_BY_NORMALIZED_PROVINCE_NAME.get((0, turkey_1.normalizeCityName)(provinceName));
    if (!plate)
        return null;
    return exports.TURKEY_AREA_CODES_BY_PLATE[plate] ?? null;
}
