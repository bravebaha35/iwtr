...
export const SECTORS: { value: string; label: string; workplaceTypes: WorkplaceType[] }[] = [
  ...
  // value stays the bare "IT" string — that's the literal Company.category
  // seeded companies carry (see apps/api/scripts/seed-*-companies.ts), and
  // the Sector filter matches on exact string equality against it.
