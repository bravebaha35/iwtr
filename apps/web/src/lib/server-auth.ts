...
// Shared by /api/auth/register, /api/auth/verify-admin-otp, and the
// dev-*-login shortcuts: all always get back a plain AuthTokensResponse on
// success, so all exchange it for httpOnly cookies the exact same way,
// instead of ever handing raw tokens back to browser JS. /api/auth/login is
// different (see exchangeLoginForSession below) — its upstream response can
// also be an OTP_REQUIRED placeholder with no tokens at all yet.
export async function exchangeCredentialsForSession(
  upstreamPath:
    | "auth/register"
    | "auth/login/verify-otp"
    | "auth/dev-admin-login"
    | "auth/dev-owner-login"
    | "auth/dev-member-login",
  body: string,
